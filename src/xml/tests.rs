...
fn test_determine_csv_xmlid_subgroup_single() {
	...
	determine_csv_xmlid_subgroup_of_xmlspan(&mut ref_at_cursor, span, 3);
...
fn test_determine_csv_xmlid_subgroup_multiple() {
	...
	let span: StrSpan = text.into();
	let mut ref_at_cursor = None;
	determine_csv_xmlid_subgroup_of_xmlspan(&mut ref_at_cursor, span, 9);
...
fn test_determine_csv_xmlid_subgroup_none() {
	...
	let span: StrSpan = text.into();
	let mut ref_at_cursor = None;
	determine_csv_xmlid_subgroup_of_xmlspan(&mut ref_at_cursor, span, text.len() + 1);
...
fn test_insert_in_scope_shadowing() {
	...
	index.insert_in_scope(&mut scope, "foo", root2, code2, false).unwrap();
	assert!(scope.get("foo").is_some());
}
